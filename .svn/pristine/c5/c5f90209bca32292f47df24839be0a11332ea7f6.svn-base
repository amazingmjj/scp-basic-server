...
public class GoodsNameService implements BaseService<GoodsNameDto, Long> {
    ...
    public void updateParentName(GoodsParentNameDto parentModel) throws Exception{
        GoodsName goodsName = new GoodsName();
        goodsName.setParentName(parentModel.getName());
        UpdateWrapper<GoodsName> qw = new UpdateWrapper<>();
        qw.eq("parent_code", parentModel.getCode());
        goodsNameMapper.update(goodsName,qw);
    }
