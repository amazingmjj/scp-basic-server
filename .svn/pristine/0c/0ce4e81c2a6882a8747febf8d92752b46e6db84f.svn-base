...
public class DptService implements BaseService<DptDto, Long> {
    ...
    public DptDto saveOrUpdate(DptDto model)  throws Exception{
        ...
        if (StringUtil.isEmpty(model.getCode())){
            ...
            Dpt oldDpt = this.selectByCode(model.getCode());
            if (oldDpt!=null&&oldDpt.getId().equals(model.getId())){
                //先校验是否被业务员引用
                if (checkReferByEmployee(entity2Dto(oldDpt))){
                    throw new Exception("已被引用不能删除");
                }
    ...
    public void delete(List<Long> ids) throws Exception {
        for (Long id : ids) {
            Dpt oldDpt = dptMapper.selectById(id);
            if (null==oldDpt){
     ...
     * @return
     */
    public boolean checkReferByEmployee(DptDto model){
        boolean refer = false;
        List<EmployeeDto> employeeList = employeeService.selectDtoByDptCode(model.getCode());
